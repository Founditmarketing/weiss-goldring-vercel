...
const handleConsultationRequest = async (message: string, userKey: string, type: string = 'text', pageUrl?: string): Promise<ConsultationResponse> => {
  try {
    ...
    for (const trace of traces) {
      ...
      // Check for Custom Action: open_calendly
      const isOpenCalendly =
        trace.type === 'open_calendly' ||
        (trace.type === 'custom_action' && trace.payload?.name === 'open_calendly') ||
        (trace.type === 'Custom' && trace.payload?.name === 'open_calendly') ||
        trace.payload?.action === 'open_calendly' ||
      ...
      if (isRedirect) {
        ...
        // If Voiceflow returns '0' or 'none', the variable was not set properly in the Voiceflow canvas.
