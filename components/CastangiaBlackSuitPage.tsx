...
interface CastangiaBlackSuitPageProps {
  onBack: () => void;
  onBook: () => void;
}

export const CastangiaBlackSuitPage: React.FC<CastangiaBlackSuitPageProps> = ({ onBack, onBook }) => {
  const [isLoaded, setIsLoaded] = useState(false);
  const location = useLocation();
  const isHighlighted = new URLSearchParams(location.search).get('highlight') === 'true';
  const [activeImage, setActiveImage] = useState('/cas-black-49338.jpg');
    ...
    <div className="bg-cream min-h-screen pt-32 pb-24" data-tags={productTags.join(', ')}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 mb-8">
        <div className="flex items-center gap-3 text-navy-800/60 font-sans text-[10px] uppercase tracking-[0.2em]">
          ...
          <span className="text-navy-900 font-medium whitespace-nowrap overflow-hidden text-ellipsis">Castangia Black Suit</span>
        </div>
      </div>
        ...
        <div className="flex flex-col lg:flex-row gap-16 lg:gap-24">
          ...
          >
            ...
            <div className="grid grid-cols-4 gap-4">
              <div 
                className={`aspect-square bg-white border cursor-pointer transition-colors overflow-hidden relative ${activeImage === '/cas-black-49338.jpg' ? 'border-gold-500 opacity-100' : 'border-navy-100 opacity-60 hover:opacity-100'}`}
                onClick={() => setActiveImage('/cas-black-49338.jpg')}
              >
                <img 
                  src="/cas-black-49338.jpg" 
                  alt="Black Suit Main" 
                  className="absolute inset-0 w-full h-full object-cover"
                />
              </div>
              <div 
                className={`aspect-square bg-white border cursor-pointer transition-colors overflow-hidden relative ${activeImage === '/cas-black-49338-details.jpg' ? 'border-gold-500 opacity-100' : 'border-navy-100 opacity-60 hover:opacity-100'}`}
                onClick={() => setActiveImage('/cas-black-49338-details.jpg')}
              >
                <img 
                  src="/cas-black-49338-details.jpg" 
                  alt="Black Suit Details" 
                  className="absolute inset-0 w-full h-full object-cover"
                />
              </div>
            </div>
